...
public class Test_GA2 extends junit.framework.TestCase {
	...
	public void test1() {
		...
		final Circle _circle = new Circle( Circle.HalfCircle );
		...
		I_EvalObject evalObject = new I_EvalObject() {
			...
			public void computeFitness(
				...
				// for each cr in the population, make it a task and put in
				// the task list of thread pool, then notify next available 
				// thread to work on the task
				for(Cr cr : population) {
					if(cr == null) {
						continue;
					}
					Task task = new Task(cr,_circle);

					tp.execute(task);
				}
				
				// each round wait until all tasks are finished, or there will
				// be null-pointer error
